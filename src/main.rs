...
fn main() {
  ...
  "provenance_limitations": [
      ...
      "Correlated against OpenROAD PDNSim across SIX routed sky130 blocks, fed the same per-instance currents and compared against a PDNSim run from the same build: worst-IR-drop ratios 1.005, 0.977, 1.004, 1.007, 1.018 and 0.916, spanning 35 uA to 1.16 mA and 2500 to 19700 grid nodes. Five of six within 2.3%; temp_sensor_apb at 0.916 is not yet explained.",
      "Precision bound: PDNSim's voltage file prints six decimals, so at these magnitudes 1 uV quantisation is a few tenths of a percent at the worst node. Agreement is to the precision the oracle publishes, and comparison below roughly the 90th percentile of the drop distribution is dominated by that quantisation rather than by either engine.",
