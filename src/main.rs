...
fn main() {
  ...
  "provenance_limitations": [
      ...
      "The solver does not converge on a large PDN. On a routed sky130 block whose power grid extracts to 5308 nodes, the Gauss-Seidel solve reaches a residual of 1.1e-7 against its 1e-8 tolerance in 50000 iterations and returns an ERROR rather than a result. The largest grid this engine is recorded as having solved is 53 nodes. Erroring is the correct failure mode -- it never hands back an unconverged number -- but real-block capability is unproven until this is fixed.",
      ...
      "Voltage sources are the power pin's port shapes where the design declares them, falling back to every pad_layer node only when it does not -- PDNSim's own precedence. Whether that fallback is optimistic or pessimistic is DESIGN-DEPENDENT and has not been measured: on the correlated block it gives 28 source nodes where the declared pin gives 683.",
