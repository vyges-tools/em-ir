...
fn main() {
  ...
  "provenance_limitations": [
      ...
      "VOLTAGES DO NOT YET AGREE: this engine UNDER-reports IR drop by about 3.2x. Fed the same per-instance currents PDNSim used, on the same block, worst drop was 4.13e-05 V against PDNSim 1.32e-04 V. Under-reporting is the dangerous direction for a checker, so a clean IR verdict here is unproven.",
      "The cause is where current enters the grid, not the resistance, which matches. Instance current lands wholly on the nearest rail node and nodes exist only at polyline points, so 15704 current-carrying instances collapse onto 834 injection nodes; a standard-cell rail is one long resistor (750 met1 resistors averaging 29.9 ohm) where PDNSim has 32603 averaging 0.685 ohm for the same total resistance. Current injected at a rail end never crosses the rail resistance the cell sits behind.",
