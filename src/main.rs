...
fn main() {
  ...
  "provenance_limitations": [
      ...
      "Correlated against OpenROAD PDNSim on one routed sky130 block with the SAME per-instance currents: worst IR drop 1.3271e-04 V against PDNSim 1.3200e-04 V, a ratio of 1.005. PDNSim's voltage file prints six decimals, so 1 uV quantisation is +/-0.76% at that node -- the supportable claim is agreement to the precision the oracle publishes, not agreement to 0.5%.",
      "Agreement decays away from the tail: p99 0.957, p90 0.918, mean 0.901, a one-sided 4-8% low. Below p90 the oracle's own quantisation dominates and the comparison stops meaning anything. Candidates for the residual, untested: PDNSim resamples nodes on a minimum pitch, splits an instance's current over every node its terminals touch where this engine uses one tap, and adds a 1 mohm ITermNode stub per terminal.",
