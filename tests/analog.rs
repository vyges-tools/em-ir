...
fn analog_bias_grid_ir_and_em() {
    ...
    assert!(
        w.voltage < rep.vdd,
        "worst node sits below VDD: {} < {}",
        w.voltage,
        rep.vdd
    );
    assert!(
        (w.drop - (rep.vdd - w.voltage)).abs() < 1e-9,
        "drop = vdd - voltage"
    );
    ...
    assert_eq!(
        w.node, "pa_m1",
        "worst IR drop at the high-current, most-distant node"
    );
    ...
    assert!(
        rep.em_checked > 0,
        "EM segments with a layer limit were checked"
    );
    // the via carrying the full 30 mA PA current is the EM hotspot (worst ratio).
    assert!(
        rep.em_worst_ratio > 0.0,
        "an EM current/limit ratio was computed"
    );

    // static (DC operating point) — no switching events, so no transient droop.
    assert!(
        rep.dynamic.is_none(),
        "static analog op-point: no `switch` events"
    );
}
